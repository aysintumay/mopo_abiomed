...
from models.policy_models import MLP
from common.logger import Logger
from common.util import set_device_and_logger

# run command: 
# python experiment_scripts/dbc.py --task halfcheetah-random-v0 --seeds 1 2 3 --model-dir saved_models/DBC --epochs 100 --device_id 5

def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--task", type=str, default="Abiomed-v0")
    parser.add_argument("--seeds", type=int, nargs='+', default=[1, 2, 3])
    parser.add_argument("--model-dir", type=str, default="dbc_models")
...
class DeterministicBehaviorCloning:
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for i in range(n_batches):
                ...
                loss = self.criterion(pred_actions, actions_batch)
                loss.backward()
                self.optimizer.step()
            ...
            if epoch > 0 and abs(avg_loss - prev_loss) < 0.0005:
...
def main():
    ...
    for seed in args.seeds:
        ...
        log_file = f'dbc_seed_{seed}_{t0}-{args.task.replace("-", "_")}'
        log_path = os.path.join(args.logdir, args.task, "dbc", log_file)
        ...
        # Train and evaluate
        dbc = DeterministicBehaviorCloning(args, logger)
        dbc.train()
        
        # Save model
        model_path = os.path.join(args.model_dir, f"dbc_{args.task}_seed_{seed}.pt")
        dbc.save_model(model_path)
        
        # Evaluate
        mean_return, std_return = dbc.evaluate()
        results.append({
            'seed': seed,
    ...
    results_path = os.path.join(args.logdir, args.task, "dbc", f"dbc_results_{t0}.csv")
